...
public class JwtServiceImpl implements JwtService {
    ...
    public UUID id(String token) {
        return UUID.fromString(decodeJWT(token).getClaim("id").asString());
    }

    public List<String> extractRoles(String token) {
        return decodeJWT(token).getClaim("roles").asList(String.class);
    }
    ...
    public Boolean isTokenExpired(String token) {
        return decodeJWT(token).getClaim("exp").asDate().before(new Date());
    }
